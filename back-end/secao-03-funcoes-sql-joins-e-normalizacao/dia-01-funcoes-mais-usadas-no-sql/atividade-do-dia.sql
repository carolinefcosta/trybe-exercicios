...
3. 🚀 Escreva uma query que exiba a média salarial de cada job_id, ordenando pela média
salarial em ordem decrescente.
...
5. 🚀 Escreva uma query que exiba quatro informações: o maior salário, o menor salário, a soma
de todos os salários e a média dos salários. Todos os valores devem ser formatados para ter apenas
duas casas decimais.
SELECT ROUND(MAX(salary), 2) AS maior_salário,
ROUND(MIN(salary), 2) AS menor_salário,
ROUND(SUM(salary), 2) AS soma_salárial,
ROUND(AVG(salary), 2) AS média_salárial
FROM hr.employees;

6. Escreva uma query que exiba a quantidade de pessoas que trabalham como pessoas programadoras
(it_prog).
SELECT job_id, COUNT(*) AS programadores FROM hr.employees
WHERE job_id = 'IT_PROG';

7. Escreva uma query que exiba a quantidade de dinheiro necessária para efetuar o pagamento de cada
profissão (job_id).
SELECT job_id, SUM(salary) AS pagamento_das_profissões
FROM hr.employees
GROUP BY job_id
ORDER BY pagamento_das_profissões;
...
13. Escreva uma query que exiba as seguintes informações de cada funcionário: id, primeiro nome e
...
WHERE DATE(hire_date) BETWEEN '1987-01-01' AND '1987-12-31'; 
...
SELECT first_name, last_name, DATEDIFF(CURRENT_DATE(), hire_date) AS dias_que_trabalham_na_empresa
FROM hr.employees;
